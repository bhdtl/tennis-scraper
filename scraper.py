...
log("🔌 Initialisiere Neural Scout (V59.5 - THE RESULT HUNTER)...")
...
def calculate_dynamic_stake(fair_prob: float, market_odds: float, ai_sentiment_score: float = 0.5) -> Dict[str, Any]:
    ...
    if 1.30 <= market_odds < 1.70:
        ...
        required_edge = 0.20 
        if ai_sentiment_score < 0.45: return {"stake_str": "0u", "type": "AI_BLOCK", "is_bet": False}
        kelly_fraction = 0.125 
        max_stake = 1.0       
        label = "💎 HUNTER"
    else: return {"stake_str": "0u", "type": "SKIP", "is_bet": False}
...
async def resolve_ambiguous_tournament(p1, p2, scraped_name, p1_country, p2_country):
    ...
    if res:
        try: 
            ...
            # Hardcoded Truth for Winston-Salem
    ...
    for day_off in range(0, 3): 
        ...
        try:
            ...
            for row in rows:
                ...
                # V59.5: Check for bold tags first (The Winner)
                winner_found = None
                bold_tags = row.find_all('b')
                
                # Check each pending match
                for pm in list(safe_to_check): # Iterate copy to allow removal
                    ...
                    # Fuzzy Check: Are both players in this row?
                    if (p1 in row_norm and p2 in row_norm):
                        
                        # 1. Bold Check (Gold Standard)
                        for b in bold_tags:
                            b_text = normalize_text(b.get_text()).lower()
                            if p1 in b_text: winner_found = pm['player1_name']
                            if p2 in b_text: winner_found = pm['player2_name']
                        
                        # 2. Left Side Check (Silver Standard)
                        if not winner_found and " - " in row_text:
                             # TennisExplorer: "Winner - Loser 6-3 6-2"
                             parts = row_text.split(" - ")
                             left_side = normalize_text(parts[0]).lower()
                             if p1 in left_side and p2 not in left_side: winner_found = pm['player1_name']
                             elif p2 in left_side and p1 not in left_side: winner_found = pm['player2_name']
                        
                        if winner_found:
                            supabase.table("market_odds").update({"actual_winner_name": winner_found}).eq("id", pm['id']).execute()
                            safe_to_check = [x for x in safe_to_check if x['id'] != pm['id']]
                            log(f"      ✅ SETTLED: {winner_found} won (vs {p2 if winner_found==pm['player1_name'] else p1})")
...
async def run_pipeline():
    log(f"🚀 Neural Scout V59.5 THE RESULT HUNTER Starting...")
