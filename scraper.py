...
log("🔌 Initialisiere Neural Scout (V83.1 - Score Master)...")
...
def extract_scores_aggressive(columns):
    ...
    for col in columns:
        ...
        # FIX: Nur einstellige Zahlen (Scores) als Int parsen
...
def determine_winner_from_scores(p1_name, p2_name, row1, row2):
    ...
    try:
        ...
        # Wenn keine Scores da sind, ist das Spiel noch nicht vorbei/begonnen
        if not s1 or not s2: return None

        p1_sets = 0
        p2_sets = 0
        # Vergleiche Satz für Satz (Spalte für Spalte)
        for k in range(min(len(s1), len(s2))):
            if s1[k] > s2[k]: p1_sets += 1
            elif s2[k] > s1[k]: p2_sets += 1
        
        # Check Retirement (Text-Suche über beide Zeilen)
        ...
        # Tennis Logic: Wer 2 Sätze hat, gewinnt (Best of 3)
...
async def scrape_single_date(browser, target_date):
    ...
    try:
        ...
        # WICHTIG: Warte kurz auf die Tabelle, falls JS sie rendert
...
def parse_matches_locally(html, p_names):
    ...
    for table in soup.find_all("table", class_="result"):
        ...
        for i in range(len(rows)):
            ...
            # Logik: Wenn Odds da sind ODER ein Gewinner gefunden wurde -> Aufnehmen
