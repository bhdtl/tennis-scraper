...
log("🔌 Initialisiere Neural Scout (V2.7 - Unstoppable + Smart Fallback)...")
...
PLAYER_CACHE: Dict[str, List[Dict]] = {} # Cache für Namen
...
def clean_tournament_name(raw: str) -> str:
    ...
    return re.sub(r'S\d+[A-Z0-9]*$', '', raw).strip()
...
# --- DER RETTER: SMART RESOLVE MIT HARD FALLBACK ---
def find_player_robust(scraped_name_raw: str) -> Optional[Dict]:
    """
    Versucht den Spieler intelligent zu finden.
    Wenn das nicht klappt, nimmt er den ersten passenden Nachnamen (wie früher).
    """
    clean_raw = clean_player_name(scraped_name_raw)
    target_last = get_last_name(clean_raw)
    ...
    if initial_match:
        ...
        for cand in candidates:
            fname = cand.get('first_name', '').lower()
...
async def get_db_data():
    try:
        ...
        # --- PLAYER CACHE AUFBAUEN ---
        global PLAYER_CACHE
        PLAYER_CACHE = {}
        for p in players:
            lname = get_last_name(p['last_name'])
            if lname:
                if lname not in PLAYER_CACHE: PLAYER_CACHE[lname] = []
...
def calculate_physics_fair_odds(p1_name, p2_name, s1, s2, bsi, surface, ai_meta, market_odds1, market_odds2):
    ...
    if bsi_val <= 4.0:
        ...
        c1_score = sum(s1.values()); c2_score = sum(s2.values())
    prob_bsi = sigmoid_prob(c1_score - c2_score, sensitivity=0.12)

    score_p1 = sum(s1.values()); score_p2 = sum(s2.values())
    prob_skills = sigmoid_prob(score_p1 - score_p2, sensitivity=0.08)

    elo1 = 1500.0; elo2 = 1500.0; elo_surf = 'Hard'
    ...
    if prob_alpha > 0.60: prob_alpha = min(prob_alpha * 1.10, 0.92)
    elif prob_alpha < 0.40: prob_alpha = max(prob_alpha * 0.90, 0.08)
...
async def build_country_city_map(browser: Browser):
    ...
    try:
        ...
        prompt = f"TASK: Map Country to City (Perth/Sydney) from text. JSON: {{ 'Germany': 'Perth', ... }} TEXT: {text_content[:20000]}"
        res = await call_gemini(prompt)
        if res:
            data = json.loads(res.replace("json", "").replace("", "").strip())
...
def parse_matches_locally(html):
    ...
    for table in tables:
        ...
        while i < len(rows):
            ...
            # WICHTIG: Wir speichern die Raw-Namen, damit find_player_robust arbeiten kann
            found.append({
...
async def update_past_results(browser: Browser):
    ...
    for pm in pending_matches:
        try:
            ...
            if (now_utc - created_at).total_seconds() / 60 > 65: safe_matches.append(pm)
        except: continue
                        ...
                        try:
                            ...
                            def get_scores(cols):
                                ...
                                for c in cols:
                                    ...
                                    if len(t)>4 or '(' in t: t=t.split('(')[0]
                                    if t.isdigit() and int(t)<=7: sc.append(int(t))
                                return sc
                            s1 = get_scores(cols1); s2 = get_scores(cols2)
                            sets1=0; sets2=0
                            for k in range(min(len(s1), len(s2))):
                                if s1[k]>s2[k]: sets1+=1
                                elif s2[k]>s1[k]: sets2+=1
                            
                            winner = None
                            if (sets1>=2 and sets1>sets2) or (is_ret and sets1>sets2):
                                if p1_last in row_text: winner = pm['player1_name']
                                elif p2_last in row_text: winner = pm['player2_name']
                            elif (sets2>=2 and sets2>sets1) or (is_ret and sets2>sets1):
                                if p1_last in next_row_text: winner = pm['player1_name']
                                elif p2_last in next_row_text: winner = pm['player2_name']
...
async def run_pipeline():
    log(f"🚀 Neural Scout v2.7 (Unstoppable Mode) Starting...")
        ...
        try:
            ...
            for day_offset in range(11): 
                ...
                for m in matches:
                    try:
                        # --- ROBUST RESOLVER ---
                        # Versucht Initialen zu matchen, sonst Fallback auf ersten Namen in DB
                        p1_obj = find_player_robust(m['p1_raw'])
                        p2_obj = find_player_robust(m['p2_raw'])
